...
QuIP~\cite{tseng2024quip_original} introduced incoherence processing for weight quantization, showing that random orthogonal rotations improve quantization bounds. QuIP\#~\cite{chee2024quip} extended this with Hadamard transforms and lattice codebooks for 2-bit quantization. While PolarQuant shares the use of Hadamard transforms, our approach differs fundamentally: QuIP\# uses incoherence to bound the worst-case quantization error, while PolarQuant uses normalization and rotation to transform the \emph{distribution} to Gaussian, enabling the use of the analytically optimal Lloyd--Max quantizer.

\subsection{TurboQuant}
\label{sec:turboquant}

TurboQuant~\cite{ashkboos2025turboquant} applies the polar quantization framework to KV cache compression during inference. It normalizes KV vectors, applies Hadamard rotation, and quantizes the resulting Gaussian coordinates with optimal centroids. Our work adapts this approach from KV cache to weight compression, demonstrates its effectiveness as a preprocessing step for downstream INT4 quantizers, and introduces the combination with AWQ for near-lossless weight compression.
